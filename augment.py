...
from crawler import get_browser  # 复用 crawler.py：反检测 + 集思录登录 + cookie 注入
...
def main():
    ...
    # 2. 初始化浏览器（复用 crawler.py：反检测 + 集思录登录 + cookie 注入，chromedriver 走本地 PATH）
    try:
        driver = get_browser(None)
    except Exception as e:
        print("错误: 无法启动浏览器，请确保已安装 chromedriver（~/.local/bin）且 jisilu_config.json 配置正确。")
        print(e)
        return
